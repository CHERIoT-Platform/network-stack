...
void __cheri_compartment("http_server_example") example()
{
	...
	while (clientsCounter < MaxClients)
	{
		...
		while (clientsCounter < MaxClients)
		{
			...
			if (ret != 0)
			{
				// This may happen if the network stack
				// crashed: if the network stack crashes during
				// `network_socket_receive` it is likely that
				// `buffer` will be a nullptr or any other
				// untagged value, and `heap_free` will return
				// an error when passed the invalid thing.
			...
			if (received > 0)
			{
				...
				while (sent < ToSend)
				{
					...
					ssize_t sentThisCall = network_socket_send(
