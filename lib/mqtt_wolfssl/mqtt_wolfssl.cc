...
//
// wolfSSL variant of the MQTT compartment. Mirrors lib/mqtt/mqtt.cc but uses
// the WolfSSLTLS compartment instead of BearSSL TLS.
...
using Debug = ConditionalDebug<DebugMQTT, "WolfSSL MQTT Client">;

struct NetworkContext
{
    WolfSSLConnection tlsHandle;
...
struct CHERIoTWolfSSLMqttContext
{
     ...
     */
    CHERIoTWolfSSLMqttContext(WolfSSLConnection  tlsHandle, AllocatorCapability allocator)
      : tlsHandle{tlsHandle}
    {
    }
     ...
     */
    ~CHERIoTWolfSSLMqttContext()
    {
        Timeout t{UnlimitedTimeout};
        wolftls_connection_close(&t, tlsHandle);
    }
...
namespace
{
     ...
     */
    __always_inline auto mqtt_wolfssl_key()

    {
        return STATIC_SEALING_TYPE(WolfSSLMQTTHandle);
    }
    ...
    int with_sendfailed_retry(Timeout                       *t,
                              ...
                              CHERIoTWolfSSLMqttContext      *connection,
                              auto                           callback)
    {
        MQTTStatus_t status;
        do
    ...
    ssize_t with_sealed_mqtt_context(Timeout               *timeout,
                                     WolfSSLMQTTConnection   sealed,
                                     auto                    callback,
                                     bool                    destructMode = false)
    {
        Sealed<CHERIoTWolfSSLMqttContext> sealedContext{sealed};
        auto *unsealed = token_unseal(mqtt_wolfssl_key(), sealedContext);
        if (unsealed == nullptr)
        {
            Debug::log("Failed to unseal WolfSSL MQTT context {}", sealed);
            return -EINVAL;
        }
        ...
        if (destructMode)
        {
            ...
            {
                Debug::log(
                  "Failed to acquire lock on WolfSSL MQTT context during close");
                return -ETIMEDOUT;
            }
        ...
        Debug::log("Failed to acquire lock on WolfSSL MQTT context");
        return -ETIMEDOUT;
    }
                           ...
                           size_t            bytesToRecv)
    {
        ...
        int32_t received = wolftls_connection_receive_preallocated(
                           ...
                           size_t            bytesToSend)
    {
        ...
        int32_t sent = wolftls_connection_send(
          &t, sealed, const_cast<void *>(sendBuffer), bytesToSend);
...
// Public WolfSSL MQTT API

WolfSSLMQTTConnection mqtt_wolfssl_connect(Timeout                   *t,
                                           ...
                                           bool                        newSession)
{
    ...
    size_t handleSize =
      sizeof(CHERIoTWolfSSLMqttContext) -
      sizeof(CHERIoTWolfSSLMqttContext::variableLengthData) + networkBufferSize +
    ...
    auto  sealedMQTTHandle = token_sealed_unsealed_alloc(
      t, allocator, mqtt_wolfssl_key(), handleSize, &unsealedMQTTHandle);
    if (sealedMQTTHandle == nullptr)
    {
        Debug::log("Failed to allocate CHERIoT WolfSSL MQTT context.");
        return nullptr;
    }

    Debug::log("Created CHERIoT WolfSSL MQTT context sealed with {}.",
               mqtt_wolfssl_key());

    // Set up a TLS stream with the broker.
    auto tlsHandle = wolftls_connection_create(
      t, allocator, hostCapability, trustAnchors, trustAnchorsCount);
    if (!Capability{tlsHandle}.is_valid())
    {
        Debug::log("Failed to open wolfSSL TLS stream.");

        // Manually destroy the handle, as we have not yet wrapped it
        // into a smart pointer.
        token_obj_destroy(allocator, mqtt_wolfssl_key(), sealedMQTTHandle);
        return nullptr;
    }

    Debug::log("Created wolfSSL TLS stream.");

    // Initialize the sealed MQTT handle.
    CHERIoTWolfSSLMqttContext *context =
      new (unsealedMQTTHandle) CHERIoTWolfSSLMqttContext{tlsHandle, allocator};
    ...
    auto cleanup = [&](void *) {
        ...
        context->~CHERIoTWolfSSLMqttContext();
    ...
    return static_cast<WolfSSLMQTTConnection>(sealedContext.release());
}

int mqtt_wolfssl_disconnect(Timeout               *t,
                            AllocatorCapability    allocator,
                            WolfSSLMQTTConnection  mqttHandle)
{
    ...
    int ret = with_sealed_mqtt_context(
      t, mqttHandle, [&](CHERIoTWolfSSLMqttContext *connection) {
    ...
    return with_sealed_mqtt_context(
      ...
      mqttHandle,
      [&](CHERIoTWolfSSLMqttContext *connection) {
          connection->~CHERIoTWolfSSLMqttContext();
          token_obj_destroy(allocator, mqtt_wolfssl_key(), mqttHandle);
          return 0;
      },
      true /* grab the context in destruct mode */);
}

int mqtt_wolfssl_publish(Timeout               *t,
                         WolfSSLMQTTConnection  mqttHandle,
                         uint8_t                qos,
                         const char            *topic,
                         size_t                 topicLength,
                         ...
                         bool                   retain)
{
    ...
    return with_sealed_mqtt_context(
      t, mqttHandle, [&](CHERIoTWolfSSLMqttContext *connection) {
          ...
          int ret =
            with_sendfailed_retry(t, "MQTT_Publish", connection, [&]() {
...
int mqtt_wolfssl_subscribe(Timeout               *t,
                           WolfSSLMQTTConnection  mqttHandle,
                           uint8_t                qos,
                           const char            *filter,
                           size_t                 filterLength)
...
int mqtt_wolfssl_unsubscribe(Timeout               *t,
                             WolfSSLMQTTConnection  mqttHandle,
                             uint8_t                qos,
                             const char            *filter,
                             size_t                 filterLength)
...
int mqtt_wolfssl_run(Timeout *t, WolfSSLMQTTConnection mqttHandle)
{
    if (!check_timeout_pointer(t))
    {
        return -EINVAL;
...
int mqtt_wolfssl_generate_client_id(char *buffer, size_t length)
{
    auto characters =
    ...
    Debug::Assert(is_valid_client_id({buffer, length}),
                  "`mqtt_wolfssl_generate_client_id` generated an invalid "
                  "client ID.");
