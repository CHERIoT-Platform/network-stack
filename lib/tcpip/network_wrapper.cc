 ...
 *
 * This is not atomic, but cannot race since only updated from the error
 * handler at a point where stricly one thread is live in the TCP/IP stack
 * compartment.
 *
 * This should not be reset by the error handler.
 */
uint64_t currentSocketEpoch = 0;
	                       ...
	                       Sealed<SealedSocket> sealedSocket)
	{
		return with_restarting_checks(
		  [&]() {
			  ...
			  if (socket->socketEpoch != currentSocketEpoch)
			  {
				  Debug::log(
				    "This socket "
...
NetworkAddress network_host_resolve(const char *hostname, bool useIPv6)
{
	return with_restarting_checks(
	  ...
	  (NetworkAddress){
	    0,
	    NetworkAddress::AddressKindInvalid} /* invalid if we are restarting */);
}
                                    ...
                                    uint16_t       localPort)
{
	return with_restarting_checks(
	  [&]() -> SObj {
		  ...
		  socketWrapper->socketEpoch = currentSocketEpoch;
...
int network_socket_close(Timeout *t, SObj mallocCapability, SObj sealedSocket)
{
	...
	return with_sealed_socket(
	  ...
	  [=](SealedSocket *socket) {
		  ...
		  if (g || (socket->socketEpoch != currentSocketEpoch))
		  {
			  if (socket->socketEpoch != currentSocketEpoch)
			  {
				  Debug::log(
...
SocketKind network_socket_kind(SObj socket)
{
	return with_restarting_checks(
	  ...
	  (SocketKind){SocketKind::Invalid, 0} /* invalid if we are restarting */);
}
