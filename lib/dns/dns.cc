...
namespace
{
	...
	ssize_t length_encoded_hostname(uint8_t *hostname, size_t maxLength)
	{
		...
		if (maxLength != 0)
		{
			if ((*(hostname + length) & 0xc0) != 0xc0)
			{
				// Skip an uncompressed name.
				uint8_t l;
				do
				{
					l = *(hostname + length);
	...
	switch (ethernetHeader->etherType)
	{
		...
		case EtherType::IPv4:
		{
			...
			else if (tcpudpHeader->sourcePort == htons(DnsServerPort))
			{
				...
				// The DNS server will echo the question and append one or more
				// answers.
				...
				while (true)
				{
					...
					else
					{
						...
						if (*(uint16_t *)(packet + currentOffset) !=
						    DNSRecordTypeCNAME)
						{
							truncated = false;
							break;
				...
				if (truncated)
				{
					Debug::log("Ignoring truncated DNS packet (hostname).");
					return;
				}
