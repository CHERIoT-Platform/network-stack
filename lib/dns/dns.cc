...
namespace
{
	 ...
	 *
	 * TODO: When should we update the MAC if we already know it?
	 */
	void process_incoming_arp_packet(uint8_t *arpPacket, size_t length)
	{
	 ...
	 *
	 * TODO: When should we update DHCP-obtained information once we have
	 * passed the initialization phase?
	 */
	void process_incoming_dhcp_packet(uint8_t        *dhcpPacket,
	                                  size_t          length,
	...
	void process_incoming_dns_packet(uint8_t *dnsPacket, size_t length)
	{
		...
		while (true)
		{
			...
			else
			{
				...
				auto type = *(uint16_t *)(dnsPacket + currentOffset);
		...
		currentOffset += 2;
		if (*(uint16_t *)(dnsPacket + currentOffset) != DNSClassIN)
		{
		...
		uint16_t ipLength = ntohs(*(uint16_t *)(dnsPacket + currentOffset));
		currentOffset += 2;
		...
		if (isIPv6)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				*ipv6++ = ntohs(*(uint16_t *)(dnsPacket + currentOffset));
				currentOffset += 2;
			}
		}
		else
		{
			...
			queryResult.ipv4 = *(uint32_t *)(dnsPacket + currentOffset);
		}
  ...
  dns_resolver_receive_frame(uint8_t *packet, size_t length)
{
	on_error(
	  ...
	  [&]() {
		  ...
		  {
			  // Unfortunately the crash happened while we were not
